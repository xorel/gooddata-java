...
class AttributeLocatorItemTest extends Specification {
    ...
    def "should serialize values"() {
        ...
        that new AttributeLocatorItem('aId', 'a1'),
                jsonEquals(resource('transform/transformation/attributeLocatorItem.json'))
    }

    def "should deserialize values"() {
        when:
        AttributeLocatorItem item = readObjectFromResource('/transform/transformation/attributeLocatorItem.json', AttributeLocatorItem)
