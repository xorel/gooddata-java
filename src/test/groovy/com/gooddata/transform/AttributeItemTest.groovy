...
class AttributeItemTest extends Specification {
    ...
    def "should serialize #set values"() {
        expect:
        that item, jsonEquals(resource("transform/${file}.json"))
    ...
    def "should deserialize all values"() {
        when:
        AttributeItem item = readObjectFromResource('/transform/attributeItemFull.json', AttributeItem)
    ...
    def "should deserialize required values"() {
        when:
        AttributeItem item = readObjectFromResource('/transform/attributeItem.json', AttributeItem)
