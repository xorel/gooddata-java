...
class AbsoluteDateFilterTest extends Specification {
    ...
    def "should serialize"() {
        ...
        that new AbsoluteDateFilter(new ObjIdentifierQualifier('date.attr'), FROM, TO),
                jsonEquals(resource("transform/absoluteDateFilter.json"))
    }

    def "should deserialize"() {
        when:
        AbsoluteDateFilter filter = readObjectFromResource('/transform/absoluteDateFilter.json', AbsoluteDateFilter)
