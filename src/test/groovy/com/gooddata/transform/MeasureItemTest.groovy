...
class MeasureItemTest extends Specification {
    ...
    def "should serialize full"() {
        ...
        that new MeasureItem(new SimpleMeasureDefinition(QUALIFIER), "lId", "alias"),
                jsonEquals(resource('transform/measureItemFull.json'))
    }

    def "should serialize"() {
        expect:
        that new MeasureItem(new SimpleMeasureDefinition(QUALIFIER)),
                jsonEquals(resource('transform/measureItem.json'))
    }

    def "should deserialize"() {
        when:
        MeasureItem measureItem = readObjectFromResource('/transform/measureItem.json', MeasureItem)
    ...
    def "should deserialize full"() {
        when:
        MeasureItem measureItem = readObjectFromResource('/transform/measureItemFull.json', MeasureItem)
